    ...
    const payload = {  
       ...
       const conversaBot =  response2.data.resource.items.map((e) => 
        ...
        pdf.create(table, options).toFile(path.resolve('./pdf/'+`history-${idProposta}.pdf`), function(err, result) {
         ...
         const fileBase = new Buffer(fs.readFileSync(path.resolve(result.filename)));
          const base64History = Base64.encode(fileBase);

          //.toString('base64'),
          //Base64.encode(JSON.stringify(conversaBot))
